...
class ProcessScanWavePickCommand extends Command {
     ...
     * @var string
     */
    protected $signature = 'ChildProcess:scan-pallet-outbound {whs_id} {host_ip} {port}';
     ...
     * @var string
     */
    protected $description = 'Call: "php artisan ChildProcess:scan-pallet-outbound [hoursBefore(default:****)]"';
            ...
            if ($resStartOperation == '0') {
                while (true) {
                    ...
                    if ($resData) {
                        ...
                        UpdateScanPalletOutboundJob::updateWavePickWMS($sock, $infoGate, $dataParse);
                    }
                }
            }
    ...
    {
        $dir = str_replace("\\","/",base_path()."/storage/logs/test-pallet-outbound-".$infoGate['host_ip'].".txt");
        $file = fopen($dir,"a");
        $i = 0;
        while(true){
